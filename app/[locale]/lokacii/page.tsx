        ...
        {venues.map((venue) => (
          <li key={venue.slug} className={venue.slug === "spomen-kosturnica" ? "md:col-span-2" : ""}>
            <Link
              ...
              className="card group block overflow-hidden transition-colors hover:border-exposure"
            >
              {venue.slug === "spomen-kosturnica" ? (
                <div className="relative aspect-[5/2] border-b-2 border-prussian bg-ink">
                  <MonumentSilhouette className="absolute inset-0 h-full w-full" />
                </div>
              ) : (
                <PhotoPending locale={locale} className="aspect-[5/2] border-0 border-b-2" />
              )}
              <div className="p-6">
                <p className="type-label text-exposure">{venue.role[locale]}</p>
                <h2 className="mt-1 text-xl font-bold text-paper group-hover:text-exposure">
                  {venue.name[locale]}
                </h2>
